...
if ($service_id) {
    ...
    if ($preselected_service) {
        // Get providers offering this service - UPDATED to use only services.duration_minutes
        ...
        // Get busy slots for these providers - UPDATED to calculate duration from services
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['csrf_token']) && $_POST['csrf_token'] === $_SESSION['csrf_token']) {
    ...
    if (empty($provider_id) || empty($service_id) || empty($date)) {
        ...
        // Get service duration and price - UPDATED to use services.duration_minutes
        $check = $conn->prepare("
        ...
        if ($result->num_rows === 0) {
            ...
            if ($overlap) {
                ...
                // Create booking - UPDATED to remove duration_minutes column
                $stmt = $conn->prepare("
...
<body>
    ...
    <script>
        // Auto-submit provider selection to show busy slots
